...
internal class ForsikringDao(private val connection: Connection) {
    ...
    internal fun lagre(forsikring: Forsikring, hyllenummer: Hyllenummer): Int {
        @Language("PostgreSQL")
        val query = """ 
            INSERT INTO forsikring (dekningsgrad, nav_overtar_ansvar_for_ventetid, premiegrunnlag, hyllenummer) 
            VALUES (:dekningsgrad, :navOvertarAnsvarForVentetid, :premiegrunnlag, :hyllenummer) 
    ...
    internal fun hent(hyllenummer: Hyllenummer): Forsikring? {
        @Language("PostgreSQL")
        val query =
            """ SELECT dekningsgrad, nav_overtar_ansvar_for_ventetid FROM forsikring WHERE behandling_id=? """
